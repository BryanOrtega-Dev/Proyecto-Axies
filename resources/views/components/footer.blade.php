<footer class="bg-footer-color pt-20 ps-[255px] pb-20 flex gap-x-[140px]">
    {{-- About Us --}}
    <div class="w-[245px] h-[202px]">
        <div class="flex gap-[10px]" >
            <svg width="30" height="41" viewBox="0 0 30 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18.75 0C10.2255 5.0344 11.25 19.2187 11.25 19.2187C11.25 19.2187 7.5 17.9375 7.5 12.1719C3.02601 14.831 0 19.9421 0 25.625C0 34.1164 6.71572 41 15 41C23.2843 41 30 34.1164 30 25.625C29.9999 13.1328 18.75 10.5703 18.75 0V0ZM16.3176 35.7023C13.3034 36.4725 10.2505 34.5924 9.49887 31.5027C8.74736 28.4131 10.5816 25.2838 13.596 24.5135C20.8732 22.6538 21.7852 18.4592 21.7852 18.4592C21.7852 18.4592 25.4141 33.3775 16.3176 35.7023Z" fill="url(#paint0_linear_201_1188)"/>
                <defs>
                <linearGradient id="paint0_linear_201_1188" x1="26.25" y1="-1.74884e-06" x2="-5.52352" y2="31.3469" gradientUnits="userSpaceOnUse">
                <stop stop-color="#E250E5"/>
                <stop offset="1" stop-color="#4B50E6"/>
                </linearGradient>
                </defs>
            </svg>
                
            <h1 class="text-white text-36px leading-[56px] font-bold">Axies</h1>
        </div>
        <p class="text-white text-justify text-14px mt-3 mb-7">Lorem ipsum dolor sit amet,consectetur
            adipisicing elit. Quis non, fugit totam vel
            laboriosam vitae.
        </p>
        <div class="flex gap-3">
            <div class="w-10 h-10 bg-secundary-grey p-2 rounded-lg">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1.00195 12.0636C1.00312 17.0133 4.58036 21.2277 9.43895 22.0034V14.9714H6.90195V12.0636H9.44195V9.85005C9.32841 8.80118 9.68447 7.75583 10.4136 6.99753C11.1427 6.23923 12.1693 5.84657 13.215 5.92605C13.9655 5.93824 14.7141 6.0055 15.455 6.12728V8.60141H14.191C13.7558 8.54407 13.3183 8.68867 13.0017 8.99449C12.6851 9.3003 12.5237 9.73423 12.563 10.174V12.0636H15.334L14.891 14.9724H12.563V22.0034C17.8174 21.1679 21.502 16.3395 20.9475 11.0163C20.3929 5.69299 15.7932 1.73835 10.4808 2.01731C5.16831 2.29626 1.0028 6.71116 1.00195 12.0636Z" fill="white"/>
                </svg>    
            </div>
            <div class="w-10 h-10 bg-secundary-grey p-2 rounded-lg">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M19.6441 6.18054C20.5012 5.64436 21.1425 4.80009 21.4483 3.80535C20.6429 4.30541 19.7618 4.65766 18.8429 4.84687C17.569 3.4367 15.5505 3.09352 13.9155 4.00911C12.2804 4.9247 11.4335 6.87238 11.848 8.76384C8.54901 8.59052 5.47544 6.95985 3.3921 4.27759C2.30485 6.23999 2.86046 8.74861 4.66182 10.0105C4.01043 9.98855 3.37348 9.804 2.80409 9.47219C2.80409 9.4902 2.80409 9.50821 2.80409 9.52622C2.80447 11.5704 4.18125 13.3312 6.09599 13.7363C5.49179 13.9083 4.85802 13.9337 4.24304 13.8104C4.78153 15.5585 6.32121 16.7561 8.07611 16.7919C6.62265 17.9856 4.82769 18.6329 2.98002 18.6298C2.65252 18.6303 2.32528 18.6106 2 18.5707C3.87627 19.8324 6.06002 20.5021 8.29028 20.4997C11.3931 20.522 14.375 19.242 16.569 16.946C18.763 14.6499 19.986 11.5295 19.9644 8.2826C19.9644 8.0965 19.9603 7.91141 19.952 7.72732C20.7556 7.11961 21.4491 6.36679 22 5.5042C21.2514 5.85144 20.4573 6.07941 19.6441 6.18054Z" fill="white"/>
                </svg>  
            </div>
            <div class="w-10 h-10 bg-secundary-grey p-2 rounded-lg">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12.2093 10.3581V13.8365H17.1483C16.6887 16.0469 14.7644 17.317 12.2093 17.317C9.2333 17.2769 6.84224 14.9062 6.84224 11.9955C6.84224 9.08482 9.2333 6.7141 12.2093 6.67404C13.4464 6.67259 14.6453 7.09241 15.6003 7.8614L18.28 5.24074C15.2061 2.59797 10.7169 2.25806 7.26368 4.40661C3.81044 6.55516 2.22311 10.6758 3.36842 14.5185C4.51374 18.3612 8.11645 21.0024 12.2093 21C16.8134 21 21 17.7252 21 11.995C20.9929 11.4433 20.9238 10.894 20.7939 10.3571L12.2093 10.3581Z" fill="white"/>
                </svg>
                    
            </div>
            <div class="w-10 h-10 bg-secundary-grey p-2 rounded-lg">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M21 2.5V14.3756L16.304 19.1249H12.391L9.954 21.5H6.913V19.1249H3V5.66616L4.227 2.5H21ZM19.435 4.08308H6.13V15.9587H9.26V18.3328L11.609 15.9577H16.304L19.434 12.7915V4.08308H19.435ZM16.305 7.24924V11.9995H14.739V7.25025H16.304L16.305 7.24924ZM12.391 7.24924V11.9995H10.826V7.25025H12.391V7.24924Z" fill="white"/>
                </svg>
                    
            </div>

        </div>
    </div>
    {{-- My Account --}}
    <div class="mt-[18px]" >
        <ul class="text-white flex flex-col gap-y-4">
            <li class="text-18px font-bold leading-[26px] mb-2"><a href="#">My Account</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Authors</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Collection</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Author Profile</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Create Collection</a></li>
        </ul>
    </div>
    {{-- Resuorces --}}
    <div class="mt-[18px]">
        <ul class="text-white flex flex-col gap-4">
            <li class="text-18px font-bold leading-[26px] mb-2"><a href="#">Resources</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Help & Support</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Live Auctions</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Item Details</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Activity</a></li>
        </ul>
    </div>
    {{-- Company --}}
    <div class="mt-[18px]">
        <ul class="text-white flex flex-col gap-4">
            <li class="text-18px font-bold leading-[26px] mb-2"><a href="#">Company</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">About Us</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Contact Us</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Our Blog</a></li>
            <li class="font-normal text-14px leading-[22px]"><a href="#">Discover</a></li>
        </ul>
    </div>
    {{-- Subscribe --}}
    <div class="ms-[5px] mt-[18px]">
        <h4 class="text-18px text-white font-bold leading-[26px] mb-2">Subscribe Us</h4>
        <form class="flex items-center mt-6" action="Post">
            <input class=" bg-transparent text-14px border-t border-b border-l border-[#343444] h-[56px] rounded-l-[10px] placeholder:text-[#8A8AA0] ps-6 text-white pe-[63px] outline-none" name="email" placeholder="Info@yourgmail.com" type="email">
            <button class="bg-blue-primary px-[18px] py-4 rounded-r-[10px]">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M3.91533 2.18876C4.23957 1.95506 4.67182 1.93688 5.01454 2.14251L20.0145 11.1425C20.3157 11.3232 20.5 11.6487 20.5 12C20.5 12.3513 20.3157 12.6768 20.0145 12.8575L5.01454 21.8575C4.67182 22.0631 4.23957 22.0449 3.91533 21.8112C3.5911 21.5775 3.43715 21.1732 3.52386 20.7831L5.25343 13L10.5 13C11.0523 13 11.5 12.5523 11.5 12C11.5 11.4477 11.0523 11 10.5 11L5.25343 11L3.52386 3.21693C3.43715 2.82677 3.5911 2.42246 3.91533 2.18876Z" fill="white"/>
                </svg>                    
            </button>
        </form>
    </div>
</footer>