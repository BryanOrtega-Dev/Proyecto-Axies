<div class="ps-[255px] py-20 flex gap-20">
    <div class="flex flex-col w-[280px]">
        <div class="py-6 border-b border-secundary-grey">
            <button class="flex items-center">
                <span class="text-white text-xl font-bold/ me-[155px]">Categories</span>
                <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11.2902 0.309656C11.1977 0.216953 11.0878 0.143405 10.9668 0.0932235C10.8459 0.0430421 10.7162 0.0172119 10.5852 0.0172119C10.4543 0.0172119 10.3246 0.043042 10.2036 0.0932235C10.0826 0.143405 9.97274 0.216952 9.88022 0.309656L6.00022 4.18965L2.12022 0.309656C1.93324 0.122678 1.67965 0.0176349 1.41522 0.0176349C1.1508 0.0176349 0.8972 0.122678 0.710222 0.309656C0.523244 0.496633 0.418203 0.750229 0.418203 1.01466C0.418203 1.27908 0.523244 1.53268 0.710222 1.71966L5.30022 6.30965C5.39274 6.40236 5.50262 6.47591 5.6236 6.52609C5.74457 6.57627 5.87425 6.6021 6.00522 6.6021C6.13619 6.6021 6.26587 6.57627 6.38685 6.52609C6.50782 6.47591 6.61771 6.40236 6.71022 6.30965L11.3002 1.71966C11.6802 1.33966 11.6802 0.699656 11.2902 0.309656Z" fill="white"/>
                </svg>
            </button>
            <div class="mt-5 flex flex-col gap-2">
                @foreach ($categories as $category)
                    <button id="filter" class="flex items-center gap-2">
                        <div id="boxOne" class="p-[2px] hidden bg-blue-primary rounded">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M8.48559 3.1255C8.3708 3.06163 8.24455 3.02101 8.11405 3.00596C7.98355 2.99092 7.85137 3.00175 7.72506 3.03783C7.59875 3.07392 7.48079 3.13454 7.37793 3.21625C7.27507 3.29795 7.18932 3.39913 7.12559 3.514L5.26909 6.855L4.20659 5.7925C4.11435 5.69699 4.004 5.62081 3.882 5.5684C3.75999 5.51599 3.62877 5.4884 3.49599 5.48725C3.36321 5.4861 3.23153 5.5114 3.10864 5.56168C2.98574 5.61196 2.87409 5.68621 2.7802 5.78011C2.68631 5.874 2.61205 5.98565 2.56177 6.10855C2.51149 6.23144 2.48619 6.36312 2.48734 6.4959C2.4885 6.62868 2.51608 6.7599 2.56849 6.88191C2.6209 7.00391 2.69708 7.11425 2.79259 7.2065L4.79259 9.2065C4.98159 9.396 5.23659 9.5 5.49959 9.5L5.63809 9.49C5.79137 9.46856 5.93757 9.41184 6.0652 9.32429C6.19283 9.23675 6.2984 9.12077 6.37359 8.9855L8.87359 4.4855C8.93742 4.37073 8.97801 4.2445 8.99306 4.11404C9.0081 3.98358 8.9973 3.85143 8.96127 3.72514C8.92524 3.59885 8.86469 3.48089 8.78307 3.37801C8.70145 3.27512 8.60037 3.18932 8.48559 3.1255Z" fill="white"/>
                            </svg>                            
                        </div>
                        <div id="boxTwo" class="border block border-white-3 p-[7px] rounded">
                        </div>
                        <span class="text-white text-[13px] leading-5 font-normal">{{$category->name}}</span>
                    </button>
                @endforeach
                
            </div>
        </div>
        <div class="pt-6">
            <button class="flex items-center">
                <span class="text-white text-xl font-bold me-[156px]">Collections</span>
                <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M11.2902 0.309656C11.1977 0.216953 11.0878 0.143405 10.9668 0.0932235C10.8459 0.0430421 10.7162 0.0172119 10.5852 0.0172119C10.4543 0.0172119 10.3246 0.043042 10.2036 0.0932235C10.0826 0.143405 9.97274 0.216952 9.88022 0.309656L6.00022 4.18965L2.12022 0.309656C1.93324 0.122678 1.67965 0.0176349 1.41522 0.0176349C1.1508 0.0176349 0.8972 0.122678 0.710222 0.309656C0.523244 0.496633 0.418203 0.750229 0.418203 1.01466C0.418203 1.27908 0.523244 1.53268 0.710222 1.71966L5.30022 6.30965C5.39274 6.40236 5.50262 6.47591 5.6236 6.52609C5.74457 6.57627 5.87425 6.6021 6.00522 6.6021C6.13619 6.6021 6.26587 6.57627 6.38685 6.52609C6.50782 6.47591 6.61771 6.40236 6.71022 6.30965L11.3002 1.71966C11.6802 1.33966 11.6802 0.699656 11.2902 0.309656Z" fill="white"/>
                </svg>
            </button>
            <div class="mt-5 flex flex-col gap-2">
                @foreach ($collections as $collection)
                    <button id="filter" class="flex items-center gap-2">
                        <div id="boxOne" class="p-[2px] hidden bg-blue-primary rounded">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M8.48559 3.1255C8.3708 3.06163 8.24455 3.02101 8.11405 3.00596C7.98355 2.99092 7.85137 3.00175 7.72506 3.03783C7.59875 3.07392 7.48079 3.13454 7.37793 3.21625C7.27507 3.29795 7.18932 3.39913 7.12559 3.514L5.26909 6.855L4.20659 5.7925C4.11435 5.69699 4.004 5.62081 3.882 5.5684C3.75999 5.51599 3.62877 5.4884 3.49599 5.48725C3.36321 5.4861 3.23153 5.5114 3.10864 5.56168C2.98574 5.61196 2.87409 5.68621 2.7802 5.78011C2.68631 5.874 2.61205 5.98565 2.56177 6.10855C2.51149 6.23144 2.48619 6.36312 2.48734 6.4959C2.4885 6.62868 2.51608 6.7599 2.56849 6.88191C2.6209 7.00391 2.69708 7.11425 2.79259 7.2065L4.79259 9.2065C4.98159 9.396 5.23659 9.5 5.49959 9.5L5.63809 9.49C5.79137 9.46856 5.93757 9.41184 6.0652 9.32429C6.19283 9.23675 6.2984 9.12077 6.37359 8.9855L8.87359 4.4855C8.93742 4.37073 8.97801 4.2445 8.99306 4.11404C9.0081 3.98358 8.9973 3.85143 8.96127 3.72514C8.92524 3.59885 8.86469 3.48089 8.78307 3.37801C8.70145 3.27512 8.60037 3.18932 8.48559 3.1255Z" fill="white"/>
                            </svg>                            
                        </div>
                        <div id="boxTwo" class="border block border-white-3 p-[7px] rounded">
                        </div>
                        <span class="text-white text-[13px] leading-5 font-normal">{{$collection->name}}</span>
                    </button>
                @endforeach
            </div>
        </div>
    </div>
    <div class="flex flex-wrap gap-[38px]">
        @foreach ($items as $item)
            @if ($loop->iteration <= 3)
                    <x-axies.itemExplore id='load-item'>
                        <x-slot name='media'>
                            <img src="{{ $item->getFirstMediaUrl() }}" class="object-cover rounded-Twenty absolute w-[290px] h-[290px]"  alt="">
                        </x-slot>
                        <x-slot name='titulo'>
                            {{$item->title}}
                        </x-slot>
                        <x-slot name='precio'>
                            {{$item->price}}
                        </x-slot>
                        <x-slot name='nameUsers'>
                            {{ $item->user->name}}
                        </x-slot>
                    </x-axies.itemExplore>
            @else 
                <x-axies.itemExplore id='load-item' class="hidden">
                    <x-slot name='media'>
                        <img src="{{ $item->getFirstMediaUrl() }}" class="object-cover rounded-Twenty absolute w-[290px] h-[290px]"  alt="">
                    </x-slot>
                    <x-slot name='titulo'>
                        {{$item->title}}
                    </x-slot>
                    <x-slot name='precio'>
                        {{$item->price}}
                    </x-slot>
                    <x-slot name='nameUsers'>
                        {{ $item->user->name}}
                    </x-slot>
                </x-axies.itemExplore>
            @endif            
        @endforeach
    </div>
</div>
<button id="load-more-button" class="w-[154px] h-[54px] mb-20 ms-[1063px] bg-transparent border border-white rounded-thirty px-10 py-4">
    <span class="text-white text-15px leading-[22px]">Load More</span>
</button>
<Script>
    let loadMoreButton = document.getElementById('load-more-button');
    let loadItems = document.querySelectorAll('[id="load-item"]');
    let currentIndex = 0;
    let itemsPerLoad = 3;

    function showNextItems() {

        loadItems.forEach((item, index) => {
        if (index >= currentIndex && index < currentIndex + itemsPerLoad) {
            item.classList.remove('hidden');
        }
        });


        currentIndex += itemsPerLoad;

        if (currentIndex >= loadItems.length) {
            loadMoreButton.classList.add('hidden');
        }
    }

    showNextItems(); 

    loadMoreButton.addEventListener('click', function () {
        showNextItems();
    });

</Script>
