<div class=" flex flex-col bg-secundary-grey rounded-Twenty p-5 w-[330px]">
    <div class="w-[290px] h-[290px] bg-white-3 rounded-Twenty relative">
        {{$media}}
        <div class="absolute top-[14px] right-3">
            <x-axies.viewLike></x-axies.viewLike>
        </div>
        <div class="absolute top-[136px] left-[73px]">
            <x-axies.buttonPlace></x-axies.buttonPlace>
        </div>
    </div>
    <div class="flex justify-between items-center mt-[21px] mb-[17px]">
        <h4 class="text-white font-bold text-18px leading-[26px]">"{{$title ?? 'Hamlet Contemplates ...'}}"</h4>
        <x-axies.bsc></x-axies.bsc>
    </div>
    <div class="flex items-center justify-between">
        <x-axies.creatorName>
            <x-slot name="nameUser">
                {{$user}}
            </x-slot>
        </x-axies.creatorName>
        <div class="flex items-end flex-col">
            <h5 class="text-white-2 text-[13px] leading-5 font-normal">Current Bid</h5>
            <h4 class="text-white text-18px leading-[26px] font-bold">{{$price}} ETH</h4> 
        </div>
    </div>
    
</div>