...
@section('main')
    ...
    <div class="px-[255px] flex flex-col">
        ...
        <div class="flex gap-x-[30px] gap-y-10 flex-wrap mt-10">
            @foreach ($items as $item) 
                @if ($loop->iteration <= 4)
                    <x-axies.itemToday id='load-item'>
                        <x-slot name='media'>
                            <img src="{{ $item->getFirstMediaUrl() }}" class="object-cover rounded-Twenty absolute w-[290px] h-[290px]"  alt="{{$item->title}}">
                        </x-slot>
                        <x-slot name='titulo'>
                ...
                @else
                    <x-axies.itemToday class="hidden" id='load-item'>
                        <x-slot name='media'>
                            <img src="{{ $item->getFirstMediaUrl() }}" class="object-cover rounded-Twenty absolute w-[290px] h-[290px]"  alt="{{$item->title}}">
                        </x-slot>
                        <x-slot name='titulo'>
        ...
        <button id="load-more-button" class="mt-[37px] w-[154px] h-[54px] mb-20 ms-[628px] bg-transparent border border-white rounded-thirty px-10 py-4">
